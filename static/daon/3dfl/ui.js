...
var startButton = document.querySelector('#btn-start-session');
...
var instrFaceElem = document.getElementById('instructions_on_face');

var panelSettings = document.getElementById('settingsdiv');

var iedElem = document.getElementById('ied');
var dimensionsElem = document.getElementById('dimensions');
var timingTrackerElem = document.getElementById('timingTracker');
var timingAnalysisElem = document.getElementById('timingAnalysis');

var showExtraInfoCheckBox = document.getElementById('showextrainfo');
const video = document.querySelector('video');

var faceBox = null;

var nbCaptures = 0;
var blackCanvas = false;
var showFaceBox = document.getElementById('displayfacebox').checked;
...
startButton.onclick = function () {
  ...
  var scoreElem = document.getElementById('score3dfl');
...
function setExtraInfoVisibility(visible) {
  var displayType = visible ? 'block' : 'none';
...
function finishSetup(video) {
  ...
  let actualConstraints = track.getConstraints();
  ...
  f3d.initialize({ mWidth, mHeight, video, onUpdate, onTemplateCreated, movementDelay: 1500, movementDuration: 3000 });
  f3d.startProcessing();
}
...
function onUpdate(updateType, additional_data) {
  ...
  switch (updateType) {
    ...
    case TYPES.FACE_BOX:
      // update ied display
      iedElem.textContent = 'face width: ' + Math.round(additional_data ? additional_data.width : '');
...
function drawRenderCanvas() {
  ...
  if (faceBox) {
    ...
    renderCanvasCtx.strokeRect(faceBox.x, faceBox.y, faceBox.width, faceBox.height);
  }
}
...
function displayVideoDimensions() {
  if (video.videoWidth) {
    dimensionsElem.innerText = '' + video.videoWidth +
      ' x ' + video.videoHeight;
  } else {
    dimensionsElem.innerText = '-- x --';
  }
}
...
function postSessionData(tpl) {
  let tplFile = new Blob([tpl], { type: 'application/octet-stream' });
  ...
  xhr('https://web3dfl-demo.identityx-cloud.com/process3dfl', formData, (res) => {
    let resJSON = null;
    if (res != null) {
...
function onServerResults(resultsObj) {
  if (resultsObj != null) {
    var liveness = resultsObj.result;
    var score3dfl = resultsObj.score.daon;
    var scorePassive = resultsObj.score.passive;
    var qualityStr = resultsObj.qualityStr;
    ...
    if (score3dfl < 0) {
      ...
      scoreElem.style.color = 'red';
      scoreElem.textContent = 'Liveness: SPOOF (score: ' + score3dfl + qualityStrOut + ')';
    } else {
      scoreElem.style.color = 'black';
      scoreElem.textContent = 'Liveness: LIVE (score: ' + score3dfl + qualityStrOut + ')';
    }
  }
  ...
  nbCaptures += 1;
  document.getElementById('nbcaptures').textContent = 'nb captures: ' + nbCaptures;
}
...
function refresh3dflTiming() {
  if (f3d.instance) {
    var processingTime = f3d.instance.getTrackerProcessingTime();
    timingTrackerElem.textContent = 'tracker: ' + Math.round(processingTime) + ' ms';

    processingTime = f3d.instance.getWorkerProcessingTime();
    timingAnalysisElem.textContent = 'full analysis: ' + Math.round(processingTime) + ' ms';
  }
}