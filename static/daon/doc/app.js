const elDocType = document.querySelector("#doc-type")
const elWidth = document.querySelector("#width")
const elUrl = document.querySelector("#url")
elUrl.value = "http://localhost:5000/https://emea.identityx-cloud.com:8087/rest/v1/quality/assessments"
...
const ID_CARD_RATIO = 1.5858;
const PASSPORT_RATIO = 1.4205;

const dc = new Daon.DocumentCapture({
  url: elUrl.value,
...
function draw() {
  const { upperLeftX, upperLeftY, width, height } = queryParams = getQueryParams();
...
function getQueryParams() {
  const overlayHeight = doc_type === "PASSPORT" ? overlayWidth / PASSPORT_RATIO : overlayWidth / ID_CARD_RATIO
  return {
    upperLeftX: Math.round((canvas.width - overlayWidth) / 2),
    upperLeftY: Math.round((canvas.height - overlayHeight) / 2) ? Math.round((canvas.height - overlayHeight) / 2) : 1,
    width: overlayWidth,
    height: Math.round(overlayHeight)
  }
}

const imgSent = document.querySelector("#img-sent");
...
function onServerFeedback(response) {
  ...
  if (typeof response === "object")
    text = JSON.stringify(response, false, 2)
  else {
    text = response
  }
  document.querySelector("pre").innerText = text;
  if (response && response.responseBase64Image) {
    document.querySelector("#img-processed").src = "data:image/png;base64," + response.responseBase64Image;
  }
}
